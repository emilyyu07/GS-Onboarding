...
def create_command(payload: CommandRequest, db: Session = Depends(get_db)):
    ...
    model = Command(command_type=payload.command_type,params=payload.params)
...
def delete_command(id: int, db: Session = Depends(get_db)):
    ...
    if item is None:
        raise HTTPException(status_code=404, detail=f"Item with id {id} was not found")  

    db.delete(item)
    db.commit()

    command_list=db.exec(select(Command)).all()

    return {"data": command_list}
